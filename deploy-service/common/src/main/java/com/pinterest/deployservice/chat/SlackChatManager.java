...
public class SlackChatManager implements ChatManager {
    ...
    private static final String PINTEREST_EMAIL_DOMAIN = "@pinterest.com";
    private String url;
    private String token;
    private Slack slack;

    public SlackChatManager(String knoxKey) {
        KnoxKeyReader keyReader = new KnoxKeyReader();
        keyReader.init(knoxKey);
        this.token = keyReader.getKey();
    ...
    private String checkAndFormatEmailAddress(String emailCandidate) {
        String trimmedEmail = emailCandidate.trim();

        if (trimmedEmail.contains("@")) {
            return trimmedEmail;
        } else {
            return trimmedEmail + PINTEREST_EMAIL_DOMAIN;
        }
    }

    private String getUserIdFromEmail(String userHandle) {
        try {
            String email = this.checkAndFormatEmailAddress(userHandle);
